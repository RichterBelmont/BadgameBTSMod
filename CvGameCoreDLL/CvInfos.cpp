...
/**
  * [Badgame] No Airships:
  * Returns if this unit is an Airship.
  */
bool CvUnitInfo::isAirship() const
{
	return m_bNoAirshipsCannotBuild;
}
...
bool CvUnitInfo::read(CvXMLLoadUtility* pXML)
{
	...
	/**
	  * [Badgame] No Airships:
	  * Reads if the unit in question is an Airship.
	  */
	pXML->GetChildXmlValByName(&m_bNoAirshipsCannotBuild, "bNoAirshipsCannotBuild", false);
...
/**
  ...
  * This is the "Am I the Apostolic Palace?" function.
  */
bool CvBuildingInfo::isApostolicPalace() const
{
	return m_bNoApostolicPalaceCannotBuild;
}

/**
  * [Badgame] No Espionage:
  * Set if this building is an Espionage-restricted building.
  */
bool CvBuildingInfo::isEspionageBuilding() const
{
	return m_bNoEspionageCannotBuild;
}

/**
  * [Badgame] No Espionage:
  * Return if this building should only be allowed to be built if this option is enabled.
  */
bool CvBuildingInfo::isBadgameEspionageBuilding() const
{
	return m_bNoEspionageCanBuild;
}
...
bool CvBuildingInfo::read(CvXMLLoadUtility* pXML)
{
	...
	/**
	  * [Badgame] No Apostolic Palace:
	  * Reads what is essentially an "Am I the Apostolic Palace?" tag from the XML
	  * and stores it.
	  */
	pXML->GetChildXmlValByName(&m_bNoApostolicPalaceCannotBuild, "bNoApostolicPalaceCannotBuild");
	/**
	  * [Badgame] No Espionage:
	  * Reads if the building has the Espionage flag set from XML.
	  */
	pXML->GetChildXmlValByName(&m_bNoEspionageCannotBuild, "bNoEspionageCannotBuild");
	/**
	  * [Badgame] No Espionage:
	  * Reads if the building is only buildable when the No Espionage flag is set.
	  */
	pXML->GetChildXmlValByName(&m_bNoEspionageCanBuild, "bNoEspionageCanBuild");
