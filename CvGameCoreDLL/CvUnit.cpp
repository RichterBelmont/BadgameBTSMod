...
bool CvUnit::canInfiltrate(const CvPlot* pPlot, bool bTestVisible) const
{
	...
	/**
	  ...
	  * This is probably redundant since we disable Spies, but
	  * no unit should be able to infiltrate, ever.
	  */
...
bool CvUnit::canEspionage(const CvPlot* pPlot, bool bTestVisible) const
{
	...
	/**
	  * [Badgame] No Espionage:
	  * Just in case there are weird units that can espionage that aren't spies,
	  * make sure that they can't do so with this option turned off.
	  */
	if (GC.getGameINLINE().isOption(GAMEOPTION_BADGAME_NO_ESPIONAGE))
	{
		return false;
