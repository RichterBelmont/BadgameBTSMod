				...
				if (GC.getCivilizationInfo(getCivilizationType()).isCivilizationFreeBuildingClass(iI))
				{
					...
					if (eLoopBuilding != NO_BUILDING)
					{
						/**
						  ...
						  * All Civs need to get the correct version of the Palace depending on the current
						  * game option setting.  Logic below is:
						  * if Badgame No Espionage is on and the building is an Espionage building, do not grant;
						  * if Badgame No Espionage is off and the building is a Badgame Espionage building, do not grant;
						  * otherwise, grant
						  */
						if (!(GC.getGameINLINE().isOption(GAMEOPTION_BADGAME_NO_ESPIONAGE) &&
							GC.getBuildingInfo(eLoopBuilding).isEspionageBuilding()) &&
							!(!GC.getGameINLINE().isOption(GAMEOPTION_BADGAME_NO_ESPIONAGE) &&
							GC.getBuildingInfo(eLoopBuilding).isBadgameEspionageBuilding()))
						{
...
int CvCity::getCommerceRateTimes100(CommerceTypes eIndex) const
{
	...
	/**
	  * [Badgame] No Espionage:
	  * No change here, we want culture to act like it does normally, without
	  * Espionage Points getting dumped into culture output.
	  */
...
void CvCity::setGreatPeopleUnitRate(UnitTypes eIndex, int iNewValue)										 
{
	...
	/**
	  * [Badgame] No Espionage:
	  * No Great Spies for us, too.
	  */

	if (GC.getGameINLINE().isOption(GAMEOPTION_BADGAME_NO_ESPIONAGE) && GC.getUnitInfo(eIndex).getEspionagePoints() > 0)
	{
        return;
	}
