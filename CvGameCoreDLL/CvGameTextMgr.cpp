			...
			if (!bPlayerContext || !(GET_PLAYER(GC.getGameINLINE().getActivePlayer()).isProductionMaxedUnitClass((UnitClassTypes)iI)))
			{
				...
				if (eLoopUnit != NO_UNIT)
				{
					if (!bPlayerContext || !(GET_PLAYER(GC.getGameINLINE().getActivePlayer()).canTrain(eLoopUnit)))
					{
						...
						{
							/**
							  * [Badgame] No Espionage + [Badgame] No Airships:
							  * Removes the "Can Train X" text from the tech picker screen when the appropriate option is set.
							  */
							if (!(GC.getUnitInfo(eLoopUnit).isSpy() && GC.getGameINLINE().isOption(GAMEOPTION_BADGAME_NO_ESPIONAGE)) &&
								!(GC.getUnitInfo(eLoopUnit).isAirship() && GC.getGameINLINE().isOption(GAMEOPTION_BADGAME_NO_AIRSHIPS)))
							{
			...
			if (!bPlayerContext || !(GET_PLAYER(GC.getGameINLINE().getActivePlayer()).isProductionMaxedBuildingClass((BuildingClassTypes)iI)))
			{
				...
				if (eLoopBuilding != NO_BUILDING)
				{
					if (!bPlayerContext || !(GET_PLAYER(GC.getGameINLINE().getActivePlayer()).canConstruct(eLoopBuilding, false, true)))
					{
						...
						{
							/**
							  * [Badgame] No Espionage + [Badgame] No Apostolic Palace:
							  * Removes the "can construct x" text from the tech picker text when mousing over
							  * the technologies that would normally let you build (N.E.) the Intelligence Agency
							  * and Security Bureau or (N.A.P.) the Apostolic Palace.
							  */
							if (!(GC.getGameINLINE().isOption(GAMEOPTION_BADGAME_NO_ESPIONAGE) &&
								  GC.getBuildingInfo(eLoopBuilding).isEspionageBuilding()) &&
								!(GC.getGameINLINE().isOption(GAMEOPTION_BADGAME_NO_APOSTOLIC_PALACE) &&
								  GC.getBuildingInfo(eLoopBuilding).isApostolicPalace()))
							{
...
void CvGameTextMgr::getEspionageString(CvWStringBuffer& szBuffer, PlayerTypes ePlayer, PlayerTypes eTargetPlayer)
{
	/**
	  ...
	  * Don't display Espionage strings.
	  */
	if (!GC.getGameINLINE().isOption(GAMEOPTION_NO_ESPIONAGE) && !GC.getGameINLINE().isOption(GAMEOPTION_BADGAME_NO_ESPIONAGE))
	{
...
void CvGameTextMgr::setCommerceHelp(CvWStringBuffer &szBuffer, CvCity& city, CommerceTypes eCommerceType)
{
	...
	/**
	  * [Badgame] No Espionage:
	  * Culture should display its old values.  No change here.
	  */
