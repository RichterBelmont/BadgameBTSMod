...
void CvPlayer::doTurn()
{
	...
	/**
	  * [Badgame] No Espionage:
	  * Set the Espionage commerce percent to zero when No Espionage is set.
	  */

	if (0 == GET_TEAM(getTeam()).getHasMetCivCount(true) || GC.getGameINLINE().isOption(GAMEOPTION_NO_ESPIONAGE) || GC.getGameINLINE().isOption(GAMEOPTION_BADGAME_NO_ESPIONAGE))
	{
...
bool CvPlayer::canTrain(UnitTypes eUnit, bool bContinue, bool bTestVisible, bool bIgnoreCost) const
{
	...
	/** 
	  * [Badgame] No Espionage:
	  * We don't want to be able to build spies, either.
	  */

	if (GC.getGameINLINE().isOption(GAMEOPTION_BADGAME_NO_ESPIONAGE))
	{
		if (GC.getUnitInfo(eUnit).isSpy() || GC.getUnitInfo(eUnit).getEspionagePoints() > 0)
		{
			return false;
	...
	/**
	  * [Badgame] No Airships:
	  * Removes Airships if that option is set.
	  */

	if (GC.getGameINLINE().isOption(GAMEOPTION_BADGAME_NO_AIRSHIPS))
	{
		if (GC.getUnitInfo(eUnit).isAirship())
		{
			return false;
		}
	}
...
bool CvPlayer::canConstruct(BuildingTypes eBuilding, bool bContinue, bool bTestVisible, bool bIgnoreCost) const
{
	...
	/**
	  * [Badgame] No Apostolic Palace:
	  * If I'm the Apostolic Palace and this game option is set, you can't build me!
	  */
	if (GC.getGameINLINE().isOption(GAMEOPTION_BADGAME_NO_APOSTOLIC_PALACE) && GC.getBuildingInfo(eBuilding).isApostolicPalace())
	{
        return false;
	}
	...
	/**
	  * [Badgame] No Espionage:
	  * Any building marked as an Espionage building is unbuildable.
	  */
	if (GC.getGameINLINE().isOption(GAMEOPTION_BADGAME_NO_ESPIONAGE) && GC.getBuildingInfo(eBuilding).isEspionageBuilding())
	{
        return false;
	}
	...
	/**
	  * [Badgame] No Espionage:
	  * Any building that is marked as a Badgame Espionage building should only be allowed to be built if
	  * this option is enabled.
	  */
	if (GC.getBuildingInfo(eBuilding).isBadgameEspionageBuilding() && !GC.getGameINLINE().isOption(GAMEOPTION_BADGAME_NO_ESPIONAGE))
	{
        return false;
	}
...
int CvPlayer::getCommerceRate(CommerceTypes eIndex) const
{
	...
	/**
	  * [Badgame] No Espionage:
	  * Culture rate is left alone here.  We don't want EPs to be added
	  * to culture.
	  */
	if (GC.getGameINLINE().isOption(GAMEOPTION_BADGAME_NO_ESPIONAGE))
	{
...
bool CvPlayer::isCommerceFlexible(CommerceTypes eIndex) const
{
	...
	if (eIndex == COMMERCE_ESPIONAGE)
	{
		/**
		  * [Badgame] No Espionage:
		  * Don't allow players to adjust commerce rates for espionage.
		  */
		if (0 == GET_TEAM(getTeam()).getHasMetCivCount(true) || GC.getGameINLINE().isOption(GAMEOPTION_NO_ESPIONAGE) || GC.getGameINLINE().isOption(GAMEOPTION_BADGAME_NO_ESPIONAGE))
		{
			return false;
...
bool CvPlayer::canDoEvent(EventTypes eEvent, const EventTriggeredData& kTriggeredData) const
{
	...
	/**
	  * [Badgame] No Espionage:
	  * Disable random events that would grant EPs.
	  */

	if (kEvent.getEspionagePoints() > 0 && GC.getGameINLINE().isOption(GAMEOPTION_BADGAME_NO_ESPIONAGE))
	{
        return false;
	}
...
UnitTypes CvPlayer::getTechFreeUnit(TechTypes eTech) const
{
	...
	/**
	  * [Badgame] No Espionage:
	  * Do not grant free Espionage units.
	  */
	if (GC.getUnitInfo(eUnit).getEspionagePoints() > 0 && GC.getGameINLINE().isOption(GAMEOPTION_BADGAME_NO_ESPIONAGE))
	{
        return NO_UNIT;
	}
