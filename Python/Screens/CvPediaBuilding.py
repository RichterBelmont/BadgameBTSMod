...
class CvPediaBuilding:
	...
	def interfaceScreen(self, iBuilding):	
		...
		# Header...
		# [Badgame] Set Civiliopedia entry names based on game option
		szHeader = ""
		if (gc.getBuildingInfo(self.iBuilding).isBadgameEspionageBuilding()):
			szHeader = u"<font=4b> [Badgame] No Espionage -- " + gc.getBuildingInfo(self.iBuilding).getDescription().upper() + u"</font>"
		else:
	...
	def getBuildingSortedList(self, bWonder):
		...
		for iBuilding in listBuildings:
			if (gc.getBuildingInfo(iBuilding).isBadgameEspionageBuilding()):
				listSorted[iI] = ("@[Badgame] No Espionage -- " + gc.getBuildingInfo(iBuilding).getDescription(), iBuilding)
			else:
