		...
		for j in range(CyGlobalContext().getNumBuildingClassInfos()):
			...
			if (eLoopBuilding != -1):
				if (isTechRequiredForBuilding(self.iTech, eLoopBuilding)):
						# [Badgame] No Espionage:
						# Option removes Intelligence Agency and Security Bureau from tech splash screen
						if not (gc.getBuildingInfo(eLoopBuilding).isEspionageBuilding() and gc.getGame().isOption(GameOptionTypes.GAMEOPTION_BADGAME_NO_ESPIONAGE)):
							...
							# Option removes Apostolic Palace from tech splash screen
							if not (gc.getBuildingInfo(eLoopBuilding).isApostolicPalace() and gc.getGame().isOption(GameOptionTypes.GAMEOPTION_BADGAME_NO_APOSTOLIC_PALACE)):
								# [Badgame] No Espionage:
								# Option removes No Espionage-specific buildings from tech splash screens
