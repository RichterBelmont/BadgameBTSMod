			...
			for j in range( gc.getNumUnitClassInfos() ):
				...
				if (eLoopUnit != -1):
					if (gc.getUnitInfo(eLoopUnit).getPrereqAndTech() == i):
						...
						# Option removes Spy icon from tech picker
						if not (gc.getUnitInfo(eLoopUnit).isSpy() and gc.getGame().isOption(GameOptionTypes.GAMEOPTION_BADGAME_NO_ESPIONAGE)):
							# [Badgame] No Espionage:
							# Option removes Airship icon from tech picker
							if not (gc.getUnitInfo(eLoopUnit).isAirship() and gc.getGame().isOption(GameOptionTypes.GAMEOPTION_BADGAME_NO_AIRSHIPS)):
			...
			for j in range(gc.getNumBuildingClassInfos()):
				...
				if (eLoopBuilding != -1):
					if (gc.getBuildingInfo(eLoopBuilding).getPrereqAndTech() == i):
						# [Badgame] No Espionage:
						# Removes the Intelligence Agency and Security Bureau buildings from the tech picker
						if not (gc.getBuildingInfo(eLoopBuilding).isEspionageBuilding() and gc.getGame().isOption(GameOptionTypes.GAMEOPTION_BADGAME_NO_ESPIONAGE)):
							# [Badgame] No Apostolic Palace:
							# Removes the Apostolic Palace from the tech picker
							if not (gc.getBuildingInfo(eLoopBuilding).isApostolicPalace() and gc.getGame().isOption(GameOptionTypes.GAMEOPTION_BADGAME_NO_APOSTOLIC_PALACE)):
							# [Badgame] No Espionage:
							# Removes Badgame No Espionage-specific buildings from the tech picker if that option is disabled
								if not (gc.getBuildingInfo(eLoopBuilding).isBadgameEspionageBuilding() and not gc.getGame().isOption(GameOptionTypes.GAMEOPTION_BADGAME_NO_ESPIONAGE)):
